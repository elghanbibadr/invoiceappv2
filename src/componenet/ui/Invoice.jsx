...
const Invoice = ({ data: { id, clientName, createdAt, total, status } }) => {
    ...
    return (
        <Link to={`/invoice/${id}`}>
            <Card onClick={handleInvoiceClicked} className=' w-[80%] mx-auto my-5  grid grid-cols-2 grid-rows-[1fr_30px_1fr] md:grid-cols-5 md:items-center md:grid-rows-1 md:h-fit   hover:border border-paleCyan '>
